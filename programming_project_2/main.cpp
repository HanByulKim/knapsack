...
int main() {
	...
	ofs << "1. Dynamic Programming(result table)" << std::endl;
...
void knapsack(int* w, int** val, int n, int m, int* vals) {
	...
	std::cout << "totalProfit : " << max << std::endl;
	std::cout << "totalWeight : " << max_m << std::endl;
	ofs << "-- result --" << std::endl;
	ofs << "totalProfit : " << max << std::endl;
	ofs << "totalWeight : " << max_m << std::endl;
}
...
void BTknapsack(int* btval, int level, int totalProfit, int totalWeight, int& maxProfit, int m, int* w, int* p, int n) {
	if (totalWeight <= m && level == n) { // answer
		...
		ofs << "Contained(1) or not(0)" << std::endl;
		for (int i = 1; i <= n; i++) { std::cout << i << " | "; ofs << i << " | "; }
		std::cout << std::endl;
		ofs << std::endl;
		for (int i = 1; i <= n; i++) { std::cout << btval[i] << " | "; ofs << btval[i] << " | "; }
		std::cout << std::endl;
		ofs << std::endl;
	}
...
void BBknapsack(int level, int totalProfit, int totalWeight, int& maxProfit, int m, int* w, int* p, int n) {
	...
	while (!a.empty()) {
		...
		if (level == n) {	// Printing Result
			...
			ofs << "totalProfit : " << totalProfit << std::endl;
			ofs << "totalWeight : " << totalWeight << std::endl;
			ofs << "bound : " << b << std::endl;
			return;
		}
